...
/** 1.发布微博 2.删除微博 3.关注用户 4.取关用户 5.获取用户微博详情 6.获取用户的初始化页面 */
public class HbaseDAO {
  ...
  public static void publishWeiBo(String uid, String content) throws IOException {
    ...
    // 5.给put对象赋值
    contPut.addColumn(
        Bytes.toBytes(Constants.CONTENT_TABLE_CF),
    ...
    for (Cell cell : result.rawCells()) {
      ...
      // 6.给收件箱表的Put对象赋值
      inboxPut.addColumn(
          Bytes.toBytes(Constants.INBOX_TABLE_CF), Bytes.toBytes(uid), Bytes.toBytes(rowKey));
  ...
  public static void addAttends(String uid, String... attends) throws IOException {
    ...
    for (String attend : attends) {
      ...
      // 5.给操作者的put对象赋值
      uidPut.addColumn(
          Bytes.toBytes(Constants.RELATION_TABLE_cf1),
      ...
      // 7.给被关注者的put对象赋值
      attendPut.addColumn(
          Bytes.toBytes(Constants.RELATION_TABLE_cf2), Bytes.toBytes(uid), Bytes.toBytes(uid));
    ...
    for (String attend : attends) {
      ...
      for (Result result : resultScanner) {
        ...
        // 6.给收件箱表的Put对象赋值
        inboxPut.addColumn(
            Bytes.toBytes(Constants.INBOX_TABLE_CF), Bytes.toBytes(attend), result.getRow());
      }
    }
  ...
  public static void deleteAttends(String uid, String... dels) throws IOException {
    ...
    for (String del : dels) {
      ...
      // 6.创建被取关者的Delete对象
      Delete deDelete = new Delete(Bytes.toBytes(del));

      // 7.给被取关者的Delete对象赋值
      deDelete.addColumn(Bytes.toBytes(Constants.RELATION_TABLE_cf2), Bytes.toBytes(uid));

      // 8.将被取关者的Delete对象添加至集合
      relaDeletes.add(deDelete);
    }
  ...
  public static void getInit(String uid) throws IOException {
    ...
    for (Cell cell : result.rawCells()) {
      ...
      for (Cell rawCell : contResult.rawCells()) {
        System.out.println(
            "RK:"
                + Bytes.toString(CellUtil.cloneRow(rawCell))
                + " CF:"
                + Bytes.toString(CellUtil.cloneFamily(rawCell))
                + " CN:"
                + Bytes.toString(CellUtil.cloneQualifier(rawCell))
                + " VALUE:"
                + Bytes.toString(CellUtil.cloneValue(rawCell)));
      }
    }
